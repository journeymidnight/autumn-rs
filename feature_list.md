...
- **Still pending (the rest of F241):**
  - F241-B (deferred): cross-SQE fan-out grouping in the daemon poller (multiple Write SQEs from one ring batch grouped by target → one `put_many` call). Lower priority than F241-A — F244-C's per-SQE FuturesUnordered already pipelines across SQEs; the framing-amortisation win is small vs the per-SQE win F241-A unlocks.
- **passes:** A done; B + cross-host perf still deferred (no quick intra-host win, awaiting a write bench harness / multi-host UCX rig). The structural-vs-perf gap is documented in F241-A perf measurement above.
