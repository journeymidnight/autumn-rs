...
- **真因（已定量）**: 单流顺序读是**每请求延迟绑定**的 —— 1840 ms / 4096 次 = **449 µs 一次
  ...
  证据：并行读者线性扩展 **1645 / 2162 / 2385 / 5341 MiB/s（1/2/4/8 流）**——8 流已经
  远超 CLI 单流的 2996，所以守护进程和集群都有大量余量，瓶颈只在单流的串行性。
- **唯一剩下的杠杆是守护进程侧预读**：检测到顺序模式时，后台预取后续 extent 进一个
  per-inode 小缓存，让下一次 FUSE 读命中本地。这是网络文件系统的标准做法，但要和 lease /
  invalidation 那套一致性机制对齐（`cached_version`、`WriterClosed` 驱逐），**属于设计决策
  而不是调参**，值得先和用户确认再动。
- **验收线可能需要重议**: 80% of CLI 单流 = 2400 MiB/s。没有守护进程预读大概率到不了；
  而按"聚合吞吐"衡量（8 流 5341）mount 已经赢了。
