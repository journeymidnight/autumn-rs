...
- **Bounded scope, two known gaps deferred to a follow-up:**
  - **Sealed-extent durability**: a flush whose imm references a previously-sealed log_stream extent (vp head crossed an extent rotation during the imm's lifetime) is still vulnerable, because seal does NOT currently fsync. The fix shape is to add `file.sync_all()` to extent-node's "applied seal from manager" path. Open as F143 if it surfaces in stress.
