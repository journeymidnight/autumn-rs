...
### F-TYPED-ERRORS — 全项目统一使用 thiserror【CLOSED：不做】
...
- **CLOSED** (2026-09-18，用户决定不做)。开工前做了一次现状盘点，结论是这个任务的
  **形状**不对，而不是收益为零：
  - thiserror **已经在用**：`common/error.rs` 的 `AppError`、`rpc/error.rs` 的 `RpcError`、
    `frame.rs`、transport、`client/lease.rs`，以及 2026-09-18 新加的
    `DiskProbeError` / `EcConvertError`。不是从零迁移。
  - `anyhow` 有 489 处、横跨 8 个 crate，但它承载的是**上下文链**，与 thiserror 定义
    **错误类型**是两件事。"统一用 thiserror"把两个角色混成一个，按字面执行等于拆掉
    489 处上下文包装 —— 巨大 diff，近零收益。
  - 真正值得改的只有**按字符串判定错误**的分类器，而且它们已经一半好了：
    `is_eversion_stale` / `is_payload_not_here` / `is_ec_gather_one_short` 走的是
    `err.chain().any(|e| e.is::<T>())`（类型化，好的）；仍在嗅字符串的是
    `is_liveness_timeout`(`contains("timed out")`)、`is_connect_failure`(`contains("connect ")`)、
    `is_short_read`、`is_owner_epoch_fence_message`、`is_transient_alloc_conflict_message`、
    `is_locked_by_other`(`{e:#}` 全链找 token)。
  - **一半的病 thiserror 治不了**：错误跨进程后没有 Rust 类型可 downcast。后三个分类器
    判的是 manager 经 RPC 回来的 message，真正的治法是**新 wire code**，等于 `MIN==MAX`
    的全停全启部署 —— 必须攒成一次 bump，不能边做边发现。前三个是本地 io/超时，
    才是便宜且纯收益的那部分。
  - 一条硬约束：`StaleVpOffset` 的 Display 是**写进文档的 wire 契约**（Python 运维工具
    正则匹配 `stale_vp_offset_past_sealed_length: extent=… offset=…`，见 stream
    CLAUDE.md note 6），机械迁移最容易悄悄踩碎它。
  **若这类缺陷将来真的咬人**（形状 = 上层按错误类型分支、下层把类型抹平、回退逻辑变
  死代码而全量测试全绿，2026-09-18 这轮在 bulk 读拒绝上抓到过一次），按血缘切片重开：
  先做本地类三个分类器（无 wire 改动，验收 = 改掉错误消息措辞后分类仍正确，还原成
  字符串实现则变红），跨进程那批单独设计并**一次性** bump；不动 anyhow 的上下文链，
  也不动只用于打日志的 Display-only 错误。
- passes: n/a（CLOSED，不做；本条不计入未完成项）
