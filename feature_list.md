...
- **passes:** false (A + D + **B** done & verified; C DEFERRED — io_uring bench harness absent + ZC ineligible at 4 KiB). Consolidation now covers the SDK primitive + kvcache (python) + fuse; only the io_uring daemon remains (its own follow-up, gated on a bench harness / F243).
