...
- **Status** (2026-09-25): 实现了 Scope (a)(c) 和 (1)(2)(4) 的接入;**(b) worker-evict 未做**;(e) 未跑。
  - 实现:`RpcClient` 内置 keepalive(`client::Keepalive`,2 s / 8 s):生命迹象 = 收到任何字节,或 TCP 上对端在我方仍有未发出积压(`tcpi_notsent_bytes>0`)时继续 ACK;静默 2 s 发 `MSG_TYPE_PING`,TCP 8–10 s / UCX ~10–12 s 判死 → `closed`+清 `pending`+释放进行中的 bulk 接收;各池按 `is_closed()` 替换。EN/PS/manager 在解码循环里直接回 ping(PS 在 authz 前)。旧服务端回 `unknown msg_type` 也算活 → 无需 wire bump。manager 的手写 `RpcConn`(另有跨 task 裸指针别名 bug)换成 `RpcClient` 池。
  - **SIGSTOP 不能证伪**:HEAD 上 SIGSTOP EN 本来就恢复(manager 停心跳判 Suspected,descriptor 不再指它),TCP/UCX 都过。所以另建了能证伪的场景 `mgr-freeze`(`scripts/fuse_dead_peer_chaos.sh` + `scripts/freeze_proxy.py`:冻结 fuse→manager 的既有流、新连接放行,再 split 让路由过期):**HEAD 每个读 30 s EIO 持续整场(3 min+)**,修复后最坏 8.3 s、之后 ≤0.2 s(各轮读取均为 6 文件并发)。根因:SDK `mgr_call` 用满 30 s `rpc_timeout`,与 fuse 30 s `REPLY_TIMEOUT` 赛跑永远输,调用被取消、死连接永不被驱逐。
  - (b):HEAD = 去掉 watchdog → mgr-freeze 全红;单元级每个机制各有消融(7 个 keepalive 测试 + 3 个服务端 ping 测试,各自消融只红目标测试)。**worker-evict 没实现**:keepalive 在 ~10 s 内关掉死连接,mgr-freeze 在没有它的情况下已全绿,按根规则 16 不加无复现需求的防御;这使 Acceptance (b) 后半句无法按原文满足 —— 待用户裁决。
  - (c):en-stop TCP+UCX 均过(UCX 最坏 13.2 s);mgr-freeze 只能 TCP(代理不能转 UCX)。
...
- **在部署之前**，若需要用 CP 构建任何镜像，必须先确认构建的是 `d6a8b73` 之前的 commit，
  或接受这次构建就是那次停机升级。
- **用户定调（2026-09-20）：不用管，到时候自己 stop-the-world 升级。**
  这条不再是任何工作的阻塞项，agent 不要再把它当紧急事项提出来。理由站得住：
  版本不匹配的拒绝是**响亮**的（握手直接拒，不是静默误读），所以 CP 那个陷阱的代价
  是一次意外停机，不是一次静默损坏——而停机本来就是这次升级的既定形态。
  排期是用户的事。
