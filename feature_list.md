...
- **Sub-parts:**
  ...
  - **F244-B (DEFERRED — env-blocked + tradeoff):** fuse `read::execute` → `get_many_into` would make its 256 KiB chunks use ZC (`get_range_into`, no per-call timeout) — the SAME robustness regression F239 was deferred for (a hung PS pins the FUSE `read()`), AND there is no FUSE-mount e2e in this env to verify a read-path change. Looping can't clear either. Revisit when a FUSE-mount test env exists (then decide timeout-safe vs full-ZC, same call as F239).
  - **F244-C (DEFERRED — env-blocked + low value):** io_uring daemon → `get_many_into`. No in-tree ioring bench/integration harness (daemon+bench are manually-run binaries), so a change to its 4 KiB 200 k-ops/s hot path can't be verified here. AND ZC is ineligible there (<64 KiB) so it's pure DRY with re-route + machinery overhead = regression risk on the hot path. Revisit with a bench harness; pairs with F243 (region UCX-registration, where ZC would actually help large reads).
...
- **passes:** false (A + D done & verified; B + C DEFERRED — environment-blocked, NOT doable in this session; not a "more time" problem). The consolidation FOUNDATION (single sub-range-capable primitive) + the kvcache caller are landed.
