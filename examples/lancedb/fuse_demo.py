...
base = unquote(urlparse(args.uri).path)
os.makedirs(base, exist_ok=True)
with tempfile.TemporaryDirectory(prefix="hardlink-", dir=base) as scratch:
    source, target, loser = [os.path.join(scratch, name) for name in ("source", "target", "loser")]
...
try:
    ...
    print("FUSE LanceDB: create, append, vector search, delete, concurrent reader/writer passed")
finally:
    db.drop_table(name)
