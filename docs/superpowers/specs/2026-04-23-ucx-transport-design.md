...
- ~~**compio I/O trait 和 UCX callback 的集成边界**~~ → **已解决 2026-04-24（P3 review fix）**：`InflightGuard` 在 future drop 时调 `ucp_request_cancel` 并同步 spin `ucp_worker_progress` 直到 cancel 回调 fire，UCX 释放对 buffer 的引用后 future 才完成 drop。回归测试 `loopback_ucx::drop_read_mid_await_is_safe` 用 `compio::time::timeout(1ms)` 强制 mid-await drop 验证。Spin 上限 100k iters，cancel 不响应时 leak request handle 而非死锁。
- ~~**`ucx-sys` crate 质量**~~ → **已解决（P0 §13）**：in-tree bindgen sub-crate `ucx-sys-mini`，零外部依赖。
- ~~**cancel 时 pending `ucp_stream_send_nb` 的 buffer 归还**~~ → **同上**，cancel-then-drain 模式保证 buffer 在 UCX 释放后才归还/drop。
- **EP 优雅 close**：`UcxConn` Drop 走 `UCP_EP_CLOSE_FLAG_FORCE`（无 flush，等价于 TCP shutdown LINGER=0；wire 上发 RST，peer 收到后视为正常断开）。生产环境如果某条 conn 上有未完成的 large append 在 in-flight，FORCE close 会丢弃 send；上层（autumn-stream）应在最后一次 append 完成（`AppendResp` 收到）后再放掉 `RpcClient` Rc。需要 graceful close 的场景应在调 Drop 前手动 flush，可在 P5 perf 暴露后再加 `UcxConn::shutdown_async()` API。
...
- ~~**eventfd-driven progress 推迟**~~ → **已解决 2026-04-24**：换成 `ucp_worker_arm + IORING_OP_POLL_ADD on efd`，event-driven 完全替代 50µs 轮询。原"chicken-and-egg"诊断错——根本原因是 UCX 1.16 的 efd 用 `O_NONBLOCK` 创建，blocking `read()` 直接 EAGAIN；正确做法是先 `poll(efd, POLLIN)` 等可读，再 `read()` 排空 counter。compio 暴露的 `PollOnce<S: AsFd>::new(fd, Interest::Readable)` 直接对应 IORING_OP_POLL_ADD，submit + await 即可。drain-before-arm + BUSY 处理按 nixl 的 `nixlUcxSharedThread::run` 同款写法。CPU 闲时真闲，wakeup 延迟 ~1 io_uring round-trip（亚微秒级）。
...
实施时改为：在 `f100-ucx` feature branch 上推进 P0–P5，全部完成后 PR 合回 `main`（per executing-plans skill safety rule）。

## 13. ucx-sys 选型（2026-04-23 P0 outcome）

**决策：in-tree bindgen 子 crate，路径 `crates/transport/ucx-sys-mini/`。**

**理由：**
- 沙箱不允许查询 crates.io 做候选筛选；user 选择 (c) "skip survey, commit to in-tree bindgen"。
- 我们只需要 ~40 个 UCP 符号，bindgen 一次性生成的代码可控；不引入外部 maintainer 依赖。
- 后续 P3 工期 +1–2 天用于把 `bindgen` 接进 `build.rs` + 通过 rustfmt 处理生成代码。

**最小符号白名单**（in-tree bindgen 必须覆盖；遗漏在 P3 编译时立刻报错）：

```
// Context
ucp_init  ucp_cleanup
ucp_config_read  ucp_config_release
ucp_context_print_info

// Worker
ucp_worker_create  ucp_worker_destroy
ucp_worker_get_efd  ucp_worker_arm  ucp_worker_progress

// Endpoint
ucp_ep_create  ucp_ep_close_nbx
...
// Stream I/O
ucp_stream_send_nbx  ucp_stream_recv_nbx

// Request lifecycle
ucp_request_cancel  ucp_request_check_status  ucp_request_free

// Status
ucs_status_string

// Types (struct + typedef)
ucp_context_h  ucp_worker_h  ucp_ep_h  ucp_listener_h  ucp_conn_request_h
ucp_params_t  ucp_worker_params_t  ucp_ep_params_t  ucp_listener_params_t
ucp_conn_request_attr_t  ucp_request_param_t  ucs_status_t
```

**Constants (FFI-generated):** `UCP_FEATURE_STREAM`, `UCP_PARAM_FIELD_FEATURES`, `UCP_WORKER_PARAM_FIELD_THREAD_MODE`, `UCS_THREAD_MODE_SINGLE`, `UCP_EP_PARAM_FIELD_{FLAGS,SOCK_ADDR,ERR_HANDLER,CONN_REQUEST}`, `UCP_EP_PARAMS_FLAGS_CLIENT_SERVER`, `UCP_LISTENER_PARAM_FIELD_{SOCK_ADDR,CONN_HANDLER}`, `UCP_CONN_REQUEST_ATTR_FIELD_CLIENT_ADDR`, `UCP_OP_ATTR_FIELD_{CALLBACK,USER_DATA}`, `UCP_OP_ATTR_FLAG_NO_IMM_CMPL`, `UCS_OK`, `UCS_INPROGRESS`, `UCS_ERR_BUSY`.

**Build host preconditions confirmed (本机):**
- `dpkg -l | grep libucx-dev` → `libucx-dev:amd64 1.16.0+ds-5ubuntu1` ✓
- `pkg-config --modversion ucx` → `1.16.0` ✓
- `clang` (bindgen runtime dep) — 待 P3 Task 14 step 1 验证；缺则 `apt install clang` (~30 MB)。

**实施位置：**
- 子 crate `crates/transport/ucx-sys-mini/` (path = "ucx-sys-mini") — `[lib] crate-type = ["rlib"]`
- `crates/transport/ucx-sys-mini/build.rs` 调 `bindgen` 输出到 `OUT_DIR/bindings.rs`
- `crates/transport/ucx-sys-mini/src/lib.rs` 仅 `include!(concat!(env!("OUT_DIR"), "/bindings.rs"));`
- `crates/transport/ucx-sys-mini/wrapper.h` 单行 `#include <ucp/api/ucp.h>`
- 父 `autumn-transport` 通过 `ucx-sys-mini = { path = "ucx-sys-mini" }` 引入，feature `ucx` 把它列为 `optional = true`
