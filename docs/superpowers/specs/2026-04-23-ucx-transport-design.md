...
- **eventfd-driven progress 推迟**：当前用 50µs 轮询（spawn 在每个 compio runtime 的后台 task 里）。spec §5.1 设想的 `ucp_worker_arm + AsyncFd` 路径在初步集成时遇到 chicken-and-egg（UCX 不 signal eventfd 直到我们 progress；progress 在 await）。简单的 drain-before-arm 修法没尝试干净，留 P5 perf 验证后再决定。50µs 轮询在 single-thread mode 占 1 个 P-log 核的~5% CPU baseline，可接受但非最优。
