...
| **M1** | Decouple FS core from `fuser` (return `InodeMeta`; feature-split `crates/fuse`; move FileAttr conv into `dispatch.rs`) | `crates/fuse` builds with `--no-default-features --features core` (no fuser); fuse binary unchanged; suite green |
| **M2** | PyO3 `autumn.Fs` binding (§4) over the core + `autumn_client::lease` | headless Rust/py test: create/readdir/read/write/rename via `Fs`; round-trips |
| **M3** | Rewrite `autumn_fsspec` as a facade over `autumn.Fs` (§7); retire `fs/` layout | offline + live fsspec suite green on the new backing; datasets round-trip green |
| **M4** | Lease integration + fencing tests (§5, §9); cross-surface interop | fencing + coherence tests green; write-via-mount/read-via-fsspec byte-identical |
