...
fn multipart_complete_is_metadata_only_and_races_resolve() {
    ...
    compio::runtime::Runtime::new().unwrap().block_on(async {
        ...
        assert!(matches!(
            multipart::complete(&mut st, up, &[(1, e1.clone()), (2, e2_old.clone()), (3, e3.clone())], Condition::None).await,
            Err(MultipartError::InvalidPart(2))
        ));
        let (ino, m) = multipart::complete(&mut st, up, &[(1, e1.clone()), (2, e2.clone()), (3, e3.clone())], Condition::Absent)
            .await
            .expect("complete");
        let mut want = p1.clone();
        want.extend_from_slice(&p2);
        want.extend_from_slice(&p3);
        assert_eq!(m.size, want.len() as u64);
        ...
        // A retried Complete after a lost reply: the record is gone now.
        assert!(matches!(
            multipart::complete(&mut st, up, &[(1, e1.clone())], Condition::None).await,
        ...
        let (ino_b, m) = multipart::complete(&mut st, up, &list, Condition::Absent).await.expect("complete without bodies");
        assert_eq!(m.size, sizes.iter().sum::<u64>());
        ...
        assert!(matches!(
            multipart::complete(&mut st, up, &[(1, s1.clone()), (2, s2.clone())], Condition::None).await,
            Err(MultipartError::EntityTooSmall(1))
        ));
        assert!(matches!(
            multipart::complete(&mut st, up, &[(2, s2.clone()), (1, s1.clone())], Condition::None).await,
            Err(MultipartError::InvalidPartOrder)
        ));
        assert!(matches!(
            multipart::complete(&mut st, up, &[(1, s1.clone()), (5, s2.clone())], Condition::None).await,
        ...
        let (only, _) = (s2.clone(), ());
        assert!(matches!(
            multipart::complete(&mut st, up, &[(2, only.clone())], Condition::Version { ino: 1, generation: 1 }).await,
        ...
        assert!(matches!(
            multipart::complete(&mut st, up, &[(2, only.clone())], Condition::None).await,
            Err(MultipartError::NoSuchUpload)
        ));
        ...
        multipart::complete(&mut st, up, &[(1, e)], Condition::None).await.unwrap();
        ...
        assert!(matches!(
            multipart::complete(&mut st, up_g, &[(1, ge.clone())], Condition::None).await,
        ...
        let (got, _) = multipart::complete(&mut st, up_h, &[(1, he.clone())], Condition::None)
            .await
        ...
        assert!(matches!(orphan.finish(&mut dead).await, Err(_)), "the dead writer is fenced out");
        let (keys, _) = rescuer
