...
mod tests {
    ...
    /// A reconcile helper: register one node so the handler can identify the
    /// reporter (an unidentified reporter gets no verdict at all).
    fn seed_reconcile_node(m: &AutumnManager, node_id: u64) {
        let mut s = m.store.inner.borrow_mut();
        s.nodes.insert(
            node_id,
            MgrNodeInfo {
                ...
                shard_ports: vec![],
                control_address: String::new(),
                node_uuid: format!("uuid-{node_id}"),
            },
        );
    }

    async fn reconcile_garbage(m: &AutumnManager, node_id: u64, ids: &[u64]) -> Vec<u64> {
        let req = rkyv_encode(&ReconcileExtentsReq {
            node_id,
            node_uuid: format!("uuid-{node_id}"),
            shard_idx: 0,
            extent_ids: ids.to_vec(),
        });
    ...
    /// An extent the manager has never heard of must NOT be condemned on the
    /// first sweep: a newly allocated extent's files exist on its nodes before
    /// `extents/<id>` is published, so a sweep answered inside that window
    /// would order the deletion of an extent that was just created. It earns
    /// the same three-round grace as the non-member case.
    #[test]
    fn reconcile_unknown_extent_gets_grace_before_garbage() {
        run(async {
            let m = AutumnManager::new();
            seed_reconcile_node(&m, 1);
            // A known extent keeps the table non-empty, so the amnesia guard
            // (tested separately) is not what is under test here.
            {
                let mut s = m.store.inner.borrow_mut();
                s.extents.insert(
                    700,
            ...
            let unknown = 999u64;
            assert!(
                !reconcile_garbage(&m, 1, &[700, unknown]).await.contains(&unknown),
                "round 1: an unknown extent must not be condemned immediately"
            );
            assert!(
                !reconcile_garbage(&m, 1, &[700, unknown]).await.contains(&unknown),
                "round 2: still inside the grace window"
            );
            assert!(
                reconcile_garbage(&m, 1, &[700, unknown]).await.contains(&unknown),
                "round 3: grace exhausted, now it is garbage"
            );
        })
    }

    /// A manager whose extent table is EMPTY while a node reports extents has
    /// lost its metadata (a memory-mode manager restart does exactly this). It
    /// cannot tell that from an empty cluster, and only one of the two makes a
    /// fleet-wide unlink correct — so it must condemn nothing, however many
    /// rounds pass.
    #[test]
    fn reconcile_issues_no_garbage_when_manager_knows_no_extents() {
        run(async {
            let m = AutumnManager::new();
            seed_reconcile_node(&m, 1);
            for round in 1..=4 {
                assert!(
                    reconcile_garbage(&m, 1, &[10, 11, 12]).await.is_empty(),
                    "round {round}: an amnesiac manager must condemn nothing"
                );
            }
        })
    }
