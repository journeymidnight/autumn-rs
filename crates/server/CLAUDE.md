...
- `--dashboard-port <P>` / `--dashboard-listen <H>` (defaults to `--listen`) / `--dashboard-allow-mutations`: embedded web dashboard + leader-fenced auto-policy controller. Read-only by default; `--dashboard-allow-mutations` arms the manual `/api/action` buttons AND lets the controller leave DryRun. autoPolicy runs only on the leader. `--auto-policy-default <NAME>` seeds an Armed policy on a fresh cluster (honored only with `--dashboard-allow-mutations`). Runbook: `docs/ops.md`.
...
| `perf-check [--threads N] [--baseline FILE] [--threshold T] [--update-baseline] [--partitions N] [--pipeline-depth K] [--group-commit-cap N]` | Regression-gated bench: pure write phase then pure read phase vs a JSON baseline; fails if throughput < threshold |
...
| Auth / tenancy | `gen-signing-key [--kid K]`, `principal-create <p> --grant ... --admin-token ...`, `principal-delete`, `principal-list`, `mint-token <p> --credential ...`, `namespace-create <name> --tenant ... [--presplit ...] --admin-token ...`, `namespace-delete`, `namespace-list` |
