//! regression: `EtcdClient` must be safe to use from multiple
//! concurrently in-flight `compio` tasks on a single-threaded runtime.
//!
//! Before the fix, `unary_call` did `self.channel.borrow_mut().call(...).await`,
...
//! the second one. Workaround was "GC one partition at a time"; the fix
...
async fn etcd_or_skip(test_name: &str) -> Option<EtcdClient> {
    match EtcdClient::connect(ETCD_ENDPOINT).await {
        Ok(c) => match c.get("__concurrent_health_check__").await {
            Ok(_) => Some(c),
            Err(e) => {
...
/// core regression: 8 concurrent `put` tasks on one EtcdClient must
...
async fn concurrent_puts_no_borrow_panic() {
    ...
    for i in 0..8 {
        let _ = client.delete(format!("__concurrent__/put/{i}")).await;
    }
    ...
    for i in 0..8 {
        ...
        handles.push(compio::runtime::spawn(async move {
            c.put(format!("__concurrent__/put/{i}"), format!("value-{i}"))
                .await
        }));
    }
    ...
    for i in 0..8 {
        let r = client.get(format!("__concurrent__/put/{i}")).await.unwrap();
...
/// mixed workload: concurrent puts + gets on one client.
#[compio::test]
async fn concurrent_mixed_no_borrow_panic() {
    ...
    for i in 0..4 {
        client
            .put(format!("__concurrent__/mix/{i}"), format!("seed-{i}"))
            .await
            .expect("seed put");
    }
    ...
    for i in 0..4 {
        let c = client.clone();
        handles.push(compio::runtime::spawn(async move {
            c.put(format!("__concurrent__/mix/{i}"), format!("upd-{i}"))
                .await
                .map(|_| ())
        }));
    }
    for i in 0..4 {
        let c = client.clone();
        handles.push(compio::runtime::spawn(async move {
            c.get(format!("__concurrent__/mix/{i}")).await.map(|_| ())
        }));
    }
    ...
    for i in 0..4 {
        let _ = client.delete(format!("__concurrent__/mix/{i}")).await;
    }
}

/// stress: 16 tasks all firing simultaneously hammer the borrow path.
#[compio::test]
async fn concurrent_stress_no_borrow_panic() {
    ...
    for i in 0..16 {
        ...
        handles.push(compio::runtime::spawn(async move {
            ...
            // borrow_mut sites if the fix regresses.
            compio::time::sleep(Duration::from_millis(1)).await;
            c.put(
                format!("__concurrent__/stress/{i}"),
    ...
    for i in 0..16 {
        let _ = client.delete(format!("__concurrent__/stress/{i}")).await;
    }
}
