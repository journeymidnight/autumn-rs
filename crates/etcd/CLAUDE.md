...
**Caller-side holding (FCI-01):** `EtcdClient` is all-`&self`, constructed once, never replaced — so callers hold it as a plain `Rc<EtcdClient>`, **not** `Rc<RefCell<EtcdClient>>`. The manager (`EtcdMirror.client`) used to wrap it in a `RefCell` and then either held a `RefMut`/`Ref` across `.await` (the F108 panic) or worked around it with `unsafe { &mut *self.client.as_ptr() }` (aliasing-UB risk). Both are gone: with `Rc<EtcdClient>`, a manager RPC is just `self.client.method().await` — no borrow to hold across the await, concurrent calls pipeline via the inner channel's sender-clone. **Do not reintroduce an outer `RefCell` around `EtcdClient`** unless you actually need to swap the whole client (you don't — it self-reconnects internally).
