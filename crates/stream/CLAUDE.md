...
| `read_bytes_from_extent(extent_id, offset, length)` | Read from extent; replication (F258): the start replica rotates by `(extent_id, offset)` hash so read IO spreads across all replicas (pre-F258 everything hit replica[0]); failover walks the remaining replicas in rotated order. **F-READ-OPENTAIL-ROTATE: OPEN-tail extents rotate too** (was replica[0]-first — the append leader — which concentrated all reads of a hot open log-tail, where fresh large values live, on one EN). Safe because an open tail's COMMITTED prefix is on every replica (all-replica-ACK); a committed VP read starts from any replica. Appends stay replica[0]-first (`launch_append`, unaffected — the rotation is in `replicated_read_order`, a read-only fn). **ZC-EXACT invariant (the rotation's companion root fix)**: the EN's ZC read (`build_read_future` zc branch) REJECTS an unservable exact-length range with `CODE_PRECONDITION` instead of `CODE_OK` + a silently short payload — `read_plan`'s clamp is only for the non-ZC scanner path whose callers handle short reads. So `CODE_OK` on MSG_READ_BYTES_ZC implies the FULL requested length; no ZC consumer needs a defensive length check. Never re-introduce a clamped-OK on the ZC path. Optional hedged read (`set_read_hedge_ms`, default 0=off, `autumn-ps --read-hedge-ms` / cluster.sh `AUTUMN_READ_HEDGE_MS`): if the first replica hasn't answered within the window, race the second and take the first Ok (eversion-stale still fails fast; loser future drop is cancel-safe). **chunked at `AUTUMN_STREAM_READ_CHUNK_BYTES` (default 256 MiB)** so reads >2 GiB don't trip the per-syscall pread ceiling on macOS (INT_MAX) / Linux (0x7ffff000); EC: parallel shard reads with decode (per-shard size already bounded). `length=0` resolves to-end via `sealed_length` (sealed extents) or `commit_length_for_extent` (open extents) before chunking. |
